...
function linkPkg (dir, val, cb) {
  ...
  console.log('Installing package ' + pkg.name + '@' + pkg.version + ' in ' + dstPath);
...
function handleResolvedPkg (dir, pkg, cb) {
  ...
  fs.stat(finalDest, function (err) {
    ...
    async.series([
      ...
      var data = require(path.join(finalDest, 'package.json'))
      var files = fs.readdirSync(finalDest)
      if (!data.scripts) {
        data.scripts = {}
      }

      if (files.indexOf('binding.gyp') > -1 && !data.scripts.preinstall) {
        data.scripts.preinstall = 'node-gyp rebuild'
      }

      if (Object.keys(data.scripts).length) {
        paths.push({ dir: finalDest, data: data });
      }

      cb(err, { pkg: pkg, data: paths })
    })
  })
}
